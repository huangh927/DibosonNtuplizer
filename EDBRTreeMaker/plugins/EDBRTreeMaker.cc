...
////////////////////////JEC for AK8/////////////////////////////////
	...
	bool doSoftDrop = iEvent.getByToken(softdropjetInputToken_, softdropjets_ );
        ...
        if( doPuppi ){
         ...
         int usenumber = -1; double pt_larger=0;
         int numvhad = puppijets_->size();
         for( int inum = 0; inum< numvhad; inum++){
           if(jetAK8puppi_pt1[inum] > pt_larger && fabs(jetAK8puppi_eta1[inum])<2.4 && inum<3) {pt_larger = jetAK8puppi_pt1[inum]; usenumber = inum; continue;}
        }

       if (usenumber>-1) {
        const pat::Jet& hadronicVpuppi = puppijets_->at(usenumber);
                jetAK8puppi_ptJEC       = jetAK8puppi_pt1[usenumber]; // unpruned corrected jet pt
                jetAK8puppi_eta     = jetAK8puppi_eta1[usenumber]; // unpruned (w/o jec) jet eta
                ...
                jetAK8puppi_sdJEC  =corr_AK8puppiSD[usenumber]*jetAK8puppi_sd;
         }
